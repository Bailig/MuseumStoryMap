...
// I add a variable for getting the location ID
...
// DETAILS_PANEL && !mobile
        if (DETAILS_PANEL && !mobile) {
            ...
            $(pDiv).append("<a href=\"#\" id=\"prev_btn\">&laquo;</a><a href=\"#\" id=\"next_btn\">&raquo;</a>");
            $(pDiv).append("<div id=\"carousel\"></div>");            
            
            $.getJSON('detailInfo.json', function(jsonObject) {
                $.each(jsonObject, function(objectName, dataArray) {
                    $.each(dataArray, function(key, value){
                        if (locationId === value.LocationID){
                        var template = ""+
                            "<div class=\"oneDetailInfo\">"+
                                "<h1 class=\"detailInfoTemplateName\">{{Name}}</h1>"+
                                "<div class=\"imgBox\">"+
                        ...
                        if (value.BeginDate !== "0" && value.EndDate !== "0"){
                            template += "<p><b>{{BeginDate}}</b> to <b>{{EndDate}}</b></p>";
                        }
                        if (value.BeginDate!== "0" && value.EndDate === "0"){
                            template += "<p><b>{{BeginDate}}</b></p>";
                        }
                        if (value.Short_Desc !== ""){
                            template += "<p><b>Description:</b> {{Short_Desc}}</p>";
                        }
                        if (value.ArtifactFinish!== ""){
                            template += "<p><b>Artifact Finish:</b> {{ArtifactFinish}}</p>";
                        }
                        if (value.Manufacturer !== ""){
                            template += "<p><b>Manufacturer:</b> {{Manufacturer}}</p>";
                        }
