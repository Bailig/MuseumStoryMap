...
function init() {
	...
	$(document).bind('cbox_complete', function(){
		$("#cboxLoadedContent .details .rightDiv").height($("#cboxLoadedContent .details").height() - $("#cboxLoadedContent .detailsTitle").height() - 50);
	});
    
    $(document).bind('cbox_closed', function(){
        $(".infoWindowLink ~ div").remove();
    });
...
function loadTemplate(LocationID,tabName,artifactID){
    var mainDiv = $('.contentPane > div');
            ...
            $.each(dataArray, function(key, value){
                if(artifactID === value.ArtifactID){
                    var template = ""+
                        ...
                        "<hr>"+
                        "<div class=\"leftDiv\">"+
                            '<div class="pictureFrame" tabindex="-1">'+ 
                                "<img src={{Pic_URL}} tabindex=\"-1\" alt=\"Picture of {{Name}}\">"+
                            '</div>'+
                        "</div>"+
                        "<div class=\"rightDiv\">";

                    if (value.BeginDate !== "0" && value.EndDate !== "0"){
                        template += "<div class=\"desc\" tabindex=\"0\"><b>{{BeginDate}}</b> to <b>{{EndDate}}</b></div><p></p>";
                    }
                    if (value.BeginDate!== "0" && value.EndDate === "0"){
                        template += "<div class=\"desc\" tabindex=\"0\"><b>{{BeginDate}}</b></div><p></p>";
                    }
                    if (value.Short_Desc !== ""){
                        template += "<div class=\"desc\" tabindex=\"0\"><b>Description:</b> {{Short_Desc}}</div><p></p>";
                    }
                    if (value.ArtifactFinish!== ""){
                        template += "<div class=\"desc\" tabindex=\"0\"><b>Artifact Finish:</b> {{ArtifactFinish}}</div><p></p>";
                    }
                    if (value.Manufacturer !== ""){
                        template += "<div class=\"desc\" tabindex=\"0\"><b>Manufacturer:</b> {{Manufacturer}}</div><p></p>";
                    }
                    if (value.Height !=="" || value.Length !=="" || value.Width !==""){
                    template += "<div class=\"desc\" tabindex=\"0\">"
                    }
                    if (value.Height !==""){
                    ...
                    template += "</div><p></p></div></div>";
                    var html = Mustache.to_html(template, value);                mainDiv.append(html);  
                }// if artifactID
            });// each value
            $.each(dataArray, function(key, value){
...
function openLightBox(){
    ...
    $(".contentPane > div .carousel").colorbox({
        ...
        open:"true",
        scrolling:false,
        onComplete: function(){_tileClick ? null : $('.address')[0].focus();},
...
// custom functions for slideshow - end
/*---------------------------------------------------------------------------------------------------------------------------------------------------------------*/
...
function showDetails(graphic, e) {
	...
	if(!graphic.attributes.getValueCI)
		graphic.attributes.getValueCI = getValueCI;
        locationID = graphic.attributes.getValueCI(FIELDNAME_LOCATIONID);
        tabName = graphic.attributes.getValueCI(FIELDNAME_TABNAME);
        artifactID = graphic.attributes.getValueCI(FIELDNAME_ARTIFACTID);
    loadTemplate(locationID,tabName,artifactID); 
}
